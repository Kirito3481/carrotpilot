...
class CarrotServ:
  ...
  def update_navi(self, remote_ip, sm, pm, vturn_speed, coords, distances, route_speed):
    ...
    if self.nSdiType  >= 0: # or self.active_carrot > 0:
      pass
      # self.debugText = (f"Atc:{atc_desired:.1f},{self.xTurnInfo}:{self.xDistToTurn:.1f}, " +
      #                   f"I({self.nTBTNextRoadWidth},{self.roadcate}) " +
      #                   f"Atc2:{atc_desired_next:.1f},{self.xTurnInfoNext},{self.xDistToTurnNext:.1f}")
      # self.debugText = "" #f" {self.nSdiType}/{self.nSdiSpeedLimit}/{self.nSdiDist},BLOCK:{self.nSdiBlockType}/{self.nSdiBlockSpeed}/{self.nSdiBlockDist}, " +
                          # "PLUS:{self.nSdiPlusType}/{self.nSdiPlusSpeedLimit}/{self.nSdiPlusDist}"
